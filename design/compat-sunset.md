...
| 2 | **hop-A legacy Basic api-key** (proxy auth fallback + key in env on non-shim path + `sc agent add` prints it) | AIK PoP path is opt-in until fail-closed | fail-closed flip (AIK sole path) + retire bearers, after e2e | core: `api_key.rs`, `proxy/handler.rs` `key_is_valid` else-branch, `cli/agent.rs` env print, `cli/run.rs` direct (non-shim) path |
