...
A vault is a security boundary, so target the right one with `--vault` (see `sc run -h`).
When the user names a vault, resolve it to an id via `sc vault ls`; if ambiguous, ask rather than guess.
